...
// create variable, then map existing array into new array, and convert each string to number using parse int, then return new array
let nums = [];
const stringsToNumbs = (numbers) => {nums = numbers.map(x => parseInt(x));}
stringsToNumbs(strNums);

// With the same numbers, find the sum of the even values

// take array of numbers, filter the even numbers and use reducer to add them together
const sumEvens = nums.filter(x => x % 2 === 0).reduce((acc, val) => acc + val);

// Find the index of the first value when added to it's index = 512 (#ATX!!)

// use find index and return once the value and index equal 512
const atxIdx = nums.findIndex((val, idx) => val + idx === 512);
